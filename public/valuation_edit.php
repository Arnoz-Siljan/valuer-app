...
// Load existing record — must belong to the current user.
$stmt = $pdo->prepare('SELECT * FROM valuations WHERE id = ? AND user_id = ?');
$stmt->execute([$id, currentUserId()]);
$record = $stmt->fetch();

if (!$record) {
    setFlash('error', 'Cenitev ni bila najdena.');
...
$errors = [];
$values = [
    'naziv_narocnika'  => $record['naziv_narocnika'],
    'naslov_narocnika' => $record['naslov_narocnika'],
    'namen_cenitve'    => $record['namen_cenitve'],
    'podlaga_vrednosti'=> $record['podlaga_vrednosti'],
    'premisa_vrednosti'=> $record['premisa_vrednosti'],
    'prvi_ogled'       => $record['prvi_ogled'],
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        setFlash('success', 'Cenitev je bila uspešno posodobljena.');
        header('Location: /valuer-app/public/dashboard.php');
        exit;
    }
}

$pageTitle = 'Uredi cenitev — Valuer.si';
